...
### training MEE
training_mean_euclidean_error_v2 = function(sl_cup_1, sl_cup_2, train_output_1, train_output_2, train_input){
  ...
  for(j in 1:length(sl_cup_1$cvFitLibrary)) { #stesso valore di sl_cup_2$cvFitLibrary
    ...
    p1_tot_training = replicate(length(train_output_1[-sl_cup_1$validRows[j][[1]]]), 0)
    for (i in 1:length(sl_cup_1$cvFitLibrary[[j]])) {
      if (coef_1[i] > 0) {
        p1_tot_training = p1_tot_training +
                          predict(sl_cup_1$cvFitLibrary[[j]][[i]], train_input[-sl_cup_1$validRows[j][[1]],],
                                  family = sl_cup_1$family) * coef_1[i]
      }
    }
    
    # calcolo le predizioni del training set dei modelli con pesi non nulli del SL2
    p2_tot_training = replicate(length(train_output_2[-sl_cup_2$validRows[j][[1]]]), 0)
    for (i in 1:length(sl_cup_2$cvFitLibrary[[j]])) {
      if (coef_2[i] > 0) {
        p2_tot_training = p2_tot_training +
                          predict(sl_cup_2$cvFitLibrary[[j]][[i]], train_input[-sl_cup_2$validRows[j][[1]],],
                                  family = sl_cup_2$family) * coef_2[i]
      }
    }
    
    # calcolo MEE per il primo fold e aggiungo il valore al vettore dei MEE
    training_mee = training_mee + mean_euclidean_error(p1_tot_training, train_output_1[-sl_cup_1$validRows[j][[1]]],
                                                       p2_tot_training, train_output_2[-sl_cup_2$validRows[j][[1]]])
  }  
...
validation_mean_euclidean_error = function(sl_cup_1, sl_cup_2, train_output_1, train_output_2, train_input){
  validation_mee = 0
  coef_1 = unname(sl_cup_1$coef)
  coef_2 = unname(sl_cup_2$coef)
  
  for(j in 1:length(sl_cup_1$cvFitLibrary)) { #stesso valore di sl_cup_2$cvFitLibrary
    p1_tot_validation = replicate(length(train_output_1[sl_cup_1$validRows[j][[1]]]), 0)
    for (i in 1:length(sl_cup_1$cvFitLibrary[[j]])) {
      if (coef_1[i] > 0) {
    ...
    p2_tot_validation = replicate(length(train_output_2[sl_cup_2$validRows[j][[1]]]), 0)
    for (i in 1:length(sl_cup_2$cvFitLibrary[[j]])) {
      if (coef_2[i] > 0) {
    ...
    validation_mee = validation_mee + mean_euclidean_error(p1_tot_validation, train_output_1[sl_cup_1$validRows[j][[1]]],
                                                           p2_tot_validation, train_output_2[sl_cup_2$validRows[j][[1]]])
  }
